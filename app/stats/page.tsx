...
export default async function StatsPage() {
  const [attempts, topicAccuracy, topics] = await Promise.all([getAttempts(), getTopicAccuracy(), getTopics()]);
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <ChartCard title="Skor per Latihan (%)">
            <ScoreTrendChart attempts={sortedDesc.slice().reverse()} />
          </ChartCard>
          <ChartCard title="Waktu Pengerjaan (menit)">
            <TimeTrendChart attempts={sortedDesc.slice().reverse()} />
          </ChartCard>
        </div>
