...
"Media Pembelajaran Zayyan & Zayna" — a Next.js learning-media/practice-question app for two elementary-school children: Zayyan (grade 3 SD, Puspresnas-olympiad-style questions) and Zayna (grade 1 SD, basic counting/vocabulary). Content and UI text are in Indonesian. Each child has their own Matematika and Bahasa Inggris subjects, scaled to their grade level. A child picks their profile, then a subject, then a topic within it (or a mixed "campuran" set scoped to that subject), takes a quiz, and sees results; an admin-gated area shows attempt history with answer keys and per-question review.
...
**Data layer** (`db/`): drizzle schema with six tables — `students` (Zayyan / Zayna, each with a `grade`), `subjects` (Matematika / Bahasa Inggris; `studentId` scopes a subject to one child — e.g. `matematika` belongs to Zayyan, `matematika-zayna` belongs to Zayna, so the same subject *name* can exist twice with different ids/content), `topics` (belongs to a subject via `subjectId`), `questions` (belongs to a topic; `multiple_choice` or `short_answer`; has `correctAnswer` + `explanation` which must never be sent to non-admin clients), `quizAttempts` (belongs to a subject via `subjectId`, plus optional `topicFilter`), and `attemptAnswers` (one row per answered question in an attempt). Subject is the top-level content scope (and transitively the per-child scope, since each subject has exactly one `studentId`): a "campuran" (mixed) quiz or the stats/topic-accuracy breakdown never crosses subjects, only topics within one subject. `subjects.studentId` is nullable at the schema level (to avoid a NOT NULL backfill migration) but every seeded subject sets it.

**`lib/` holds all query/business logic**, kept separate from routes/pages so both server components and API routes can reuse it:
- `lib/students.ts` — `getStudents()`.
- `lib/subjects.ts` — `getSubjects(studentId?)`; no argument returns all subjects (used for admin/stats spanning both children), with a `studentId` it filters to that child's subjects.
- `lib/subject-style.ts` — `subjectEmoji`/`subjectColorClasses` keyed by subject *name* prefix (not id), so Zayyan's and Zayna's same-named subjects (e.g. both called "Matematika") get consistent styling without a per-id map.
...
- `lib/admin-review.ts` — `getAttemptReview` joins an attempt with its answers, questions, correct answers, and explanations (the one place full answer keys are assembled, only called from admin-gated code paths); also `deleteAttempt` (removes an attempt and its `attemptAnswers`).
...
**Routing**: `/` is a student/child picker (from `getStudents()`); `/[student]` is that child's subject picker (from `getSubjects(studentId)`); `/[student]/[subject]` is the topic picker (from `getTopics(subjectId)`, after validating the subject actually belongs to that student), linking into `/quiz/[subject]/[topicOrMixed]`. The quiz/result routes stay keyed by `subjectId` alone (no `student` segment) since subject ids are already unique per child.
...
**Charts**: `app/stats/StatsCharts.tsx` uses Recharts for score/time trend and per-topic accuracy visualizations. `app/stats/page.tsx` renders these three charts once per subject (each fed by `getAttempts(subjectId)` / `getTopicAccuracy(subjectId)`) plus one combined "Riwayat Latihan" history table (unfiltered, with a Mata Pelajaran column) below. Since Zayyan and Zayna each have a subject named "Matematika" and one named "Bahasa Inggris", section headers and the history table's subject column prefix the subject name with the owning student's name (via a `studentId -> name` map) to disambiguate — see `subjectNameById` construction in `app/stats/page.tsx` and `app/admin/page.tsx`.
